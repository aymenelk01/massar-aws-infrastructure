# Create an SQS queue to be used as a dead-letter queue (DLQ) for the documents system.
resource "aws_sqs_queue" "dlq" {
  name                      = "${var.environment}-documents-dlq"
...
# Create the main SQS queue for the documents system, which will use the DLQ for messages that fail processing.
resource "aws_sqs_queue" "main" {
  name                       = "${var.environment}-documents-queue"
