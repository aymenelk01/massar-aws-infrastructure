...
- **Lambda Post-Confirmation Trigger for Admin Provisioning:** While the bootstrap script resolves the Cognito password-in-state risk, a production-grade architecture should avoid client-side provisioning scripts. The planned improvement involves enabling self-sign-up on the Cognito User Pool and deploying an AWS Lambda function triggered by the Post Confirmation Cognito event. The Lambda function inspects the confirmed user's email server-side and automatically promotes the designated administrator to the admins Cognito group — eliminating the post-apply script dependency from the CI/CD pipeline entirely.
